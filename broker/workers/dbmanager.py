...
from dbtools import load_products, save_products
...
class DBmanager:
    ...
    def backup(self):
        ...
        dst_file = 'market_bak/market_'+datetime.today().strftime('%Y%m%d%H%M%S')+'.hdf5'
    ...
    def __marketinfo(self, code):
        if self.tr.name != 'request_productsinfo':
            return
        ...
        for group in self.products.values():
            group['codes'].sort(key=lambda x: x['month'])
            group['codes'] = group['codes'][:3] #최근 3개 월물만 저장

        self.allcodes = [x for grp in self.products.values() for x in grp['codes']]
    ...
    def __collect_trade_data(self, code):
        if self.tr.name == 'collect_trade_data':
        ...
        op = cl = vol = 0
        for i in range(cnt):
            op += float(self.query.get_field_data(outblock, 'open', i)) #시가
            cl += float(self.query.get_field_data(outblock, 'close', i)) #종가
        ...
        self.code['ec_price'] = round((op+cl)/(2*cnt), digit)  if cnt else 0
    ...
    def set_active(self):
        for group in self.products.values():
            group['active'] = deepcopy(max(group['codes'], key=lambda x: x['volume']))
            group['active']['activated_date'] = self.today
            group['active']['price_gap'] = 0
    ...
    def verification(self):
        old = load_products()
        if old:
            for group, product in self.products.items():
                # case 1. 신규 생성된 상품
                if group not in old:
                    continue
                ...
                # case 3. 신규 액티브 월물이 갱신된 경우: 갱신
                    ...
                    newprice = [x['ec_price'] for x in old[group]['codes'] if x['code'] == cur_ac['code']][0]
                    lastprice = [x['ec_price'] for x in old[group]['codes'] if x['code'] == old_ac['code']][0]
                    cur_ac['price_gap'] = round(newprice - lastprice, product['decimals'])
    ...
    def get_ohlc_data(self):
        ...
        grp = self.product['group']
        self.cursor = getattr(self.h5file.root, grp).OHLC #db 커서
        ...
        if self.product['active']['code'] != active:
            ...
            self.message.append("!!! %s Data Has been changed up by %s from %s"%\
                             (self.product['name'], price_gap, self.lastday.astype('M8[s]')))
    ...
    def _on_get_ohlc_data(self, code):
        ...
        if data:
            msg = "Updating daily: %s at  %s, TR: %s, (%s/%s)"\
                  %(self.product['name'], sdate, count, len(self.products_l), self.codelength)
    ...
    def get_density_data(self):
        ...
        self.cursor = getattr(self.h5file.root, grp).Density
        self.lastdate = max(self.cursor.cols.date, default=np.array(0)) #최근 저장된 날짜
        ...
        if self.product['active']['code'] != active:
            ...
            self.message.append("!!!CASE1: %s Data Has been changed up by %s from %s"%\
                             (self.product['name'], price_gap, self.lastdate.astype('M8[s]')))
        ...
        self.logger.info("Started to get MINUTE data : %s upto %s",
                         self.product['name'], self.lastdate.astype('M8[s]'))
    ...
    def _on_get_density_data(self, code):
        if self.tr.name != 'get_density_data':
            return 
        shcode = self.query.get_field_data(self.tr.OUTBLOCK, 'shcode', 0) #종목코드
        cts_date = self.query.get_field_data(self.tr.OUTBLOCK, 'cts_date', 0) #연속일자
        ...
        for i in range(cnt):
            ...
            if self.cursor.read_where('date==ndate').size:
                ...
                tickunit = self.product['tick_unit']
                
                if round(low, digit) == round(high, digit):
                    item = (ndate, round(low, digit), volume)
                    items.append(item)
                    ...
                    for price in np.arange(round(low, digit), high - tickunit/2, tickunit):
                        item = (ndate, round(price, digit), value)
                        items.append(item)
